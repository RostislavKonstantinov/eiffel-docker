...
#Branch 1
if [ "$output" = "" ]; then
    time -p timeout "$to" $compiler /usercode/$file -< $"/usercode/inputFile" #| tee /usercode/output.txt
#Branch 2
else
  ...
  $compiler /usercode/$file $addtionalArg #&> /usercode/errors.txt
	#Branch 2a
	if [ $? -eq 0 ];	then
	    if [ -d "/usercode/EIFGENs" ]; then
	       exec  2> $"/usercode/errors"
	       exec  1> $"/usercode/logfile.txt"
	       chmod +x "/usercode/application"
	       time -p timeout "$to" ./application -< $"/usercode/inputFile"
	    else
	       time -p timeout "$to" $output -< $"/usercode/inputFile" #| tee /usercode/output.txt
	    fi
	#Branch 2b
	else
